...
export default function Carousel({ children, refContainer }) {
  ...
  useLayoutEffect(() => {
    const refForwardHandler = refDragHandler.current;

    refForwardHandler.onmousedown = onDragStart;
    refForwardHandler.addEventListener("touchstart", onDragStart);
    refForwardHandler.addEventListener("touchend", onDragEnd);
    refForwardHandler.addEventListener("touchmove", onDragMove);
    refForwardHandler.addEventListener("click", onClick);
    refForwardHandler.addEventListener("transitionend", fnCheckIndex);
    return () => {
      refForwardHandler.removeEventListener("touchstart", onDragStart);
      refForwardHandler.removeEventListener("touchend", onDragEnd);
      refForwardHandler.removeEventListener("touchmove", onDragMove);
      refForwardHandler.removeEventListener("click", onClick);
      refForwardHandler.removeEventListener("transitionend", fnCheckIndex);
    };
  }, [onDragStart, onDragEnd, onDragMove, fnCheckIndex, onClick]);
